...
int locationforecast_formatter(void) {
  ...
  for (int i = 0; i < n_timeseries; i++) {
    ...
    printf("%s\n", json_object_get_string(symbol_code_obj));
    printf("%s ⁰C\n", json_object_get_string(air_temperature_obj));
    printf("%s ", degrees_to_compass_directions(json_object_get_double(wind_from_direction_obj)));
    printf("%.1f(%.1f) m/s\n", json_object_get_double(wind_speed_obj), json_object_get_double(wind_speed_gust_obj));
    printf("%.1f mm\n", json_object_get_double(precipitation_amount_obj));
  }
  return 0;
}
