...
int locationforecast_formatter(void) {
  ...
  struct json_object *json_obj, *properties_obj, *timeseries_obj, *time_obj,
      ...
      *wind_speed_gust_obj, *next_6_hours_obj, *summary_obj, *symbol_code_obj, *precipitation_amount_obj;
  ...
  for (int i = 0; i < n_timeseries; i++) {
    ...
    exists =
        json_object_object_get_ex(instant_details_obj, "wind_speed", &wind_speed_obj);
    if (exists == false) {
      fprintf(stderr, "wind_speed not found\n");
      return -1;
    }
    exists = json_object_object_get_ex(instant_details_obj, "wind_speed_of_gust",
                                       &wind_speed_gust_obj);
    if (exists == false) {
      fprintf(stderr, "wind_from_direction not found\n");
      return -1;
    ...
    exists = json_object_object_get_ex(next_6_hours_obj, "details", &details_obj);
    if (exists==false) {
